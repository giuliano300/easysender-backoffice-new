...
export class TotalReportComponent {
  ...
  displayedColumns: string[] = ['userComplete', 'rol', 'rolPrice', 'lol', 'lolPrice', 'tol', 'tolPrice', 'mol', 'molPrice', 'col', 'colPrice', 
    'vol', 'volPrice', 'agol', 'agolPrice', 'sends'];
  ...
  dataSource = new MatTableDataSource<TotalReport>(this.reports);
  ...
  getTotalReports(filter: any = "") {
    ...
    this.sendService.getTotalReport(filter).subscribe({
      next: (data: TotalReport[]) => {
        ...
        this.costoTotale = this.reports.reduce((sum, r) => sum + r.rolPrice + r.lolPrice+ r.molPrice + r.colPrice + r.tolPrice + r.volPrice + r.agolPrice, 0);
      },
      error: (error) => {
        if (error.status === 404) {
          this.reports = [];
